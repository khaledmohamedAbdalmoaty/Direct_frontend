...
export default function UploadFileComponent({actiontype,postTitle,inputPlaceHolder,TooltipTitle}) {
 ...
 const {mutate}=useUpLoadConversationImage()
  ...
  const handleSubmit=async (e)=>{
    e.preventDefault()
    try{
      const imageLocaton=await UploadImageToMulter(file)
      console.log(`image location that come from first step => ${imageLocaton}`)
      mutate({channelId,userId,imageLocaton,imageCaption})
