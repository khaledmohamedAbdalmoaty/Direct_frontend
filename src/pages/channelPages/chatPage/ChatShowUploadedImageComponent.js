/* -------------------------------------------------------------------------- */
/*                  import things related to react technology                 */
/* -------------------------------------------------------------------------- */
import React from 'react'

/* -------------------------------------------------------------------------- */
/*                     import things from material mui.v5                     */
/* -------------------------------------------------------------------------- */

import Card from '@mui/material/Card';
import CardHeader from '@mui/material/CardHeader';
import CardMedia from '@mui/material/CardMedia';
import CardContent from '@mui/material/CardContent';
import Typography from '@mui/material/Typography';
import Avatar from '@mui/material/Avatar';

/* import CardActions from '@mui/material/CardActions';
import Collapse from '@mui/material/Collapse';
import IconButton from '@mui/material/IconButton';
import { styled } from '@mui/material/styles';
import AddCommentIcon from '@mui/icons-material/AddComment';
import {Button} from '@mui/material'

 */

/* import { red } from '@mui/material/colors';
import FavoriteIcon from '@mui/icons-material/Favorite';
import ShareIcon from '@mui/icons-material/Share';
import ExpandMoreIcon from '@mui/icons-material/ExpandMore';
import MoreVertIcon from '@mui/icons-material/MoreVert';

import Checkbox from '@mui/material/Checkbox';
import FavoriteBorder from '@mui/icons-material/FavoriteBorder';
import Favorite from '@mui/icons-material/Favorite';
import BookmarkBorderIcon from '@mui/icons-material/BookmarkBorder';
import BookmarkIcon from '@mui/icons-material/Bookmark'; */

/* -------------------------------------------------------------------------- */
/*               import things related to react router dom var.6              */
/* -------------------------------------------------------------------------- */
import { useNavigate} from 'react-router-dom';

/* const image_location=`localhost:8800/images/1651155848883Screenshot from 2022-04-12 14-15-35.png`
 */

/* -------------------------------------------------------------------------- */
/*                           import custom function                           */
/* -------------------------------------------------------------------------- */
import axios from 'axios' 

/* -------------------------------------------------------------------------- */
/*                         import adjust date function                        */
/* -------------------------------------------------------------------------- */
import {ReturnDate} from '../../../common/ReturnDate'


const image_location='/images/try_image.png'
const image_location_multer="http://localhost:8800/images/165572985671813.png"
/* const image_location_multer="media/khaled/Disk_2_319G/1/graduation_project/backend/public/images/165572985671813.png"
 *//* const image_location_multer="data:image/jpeg;base64,"
 */
const ChatShowUploadedImageComponent = ({msg}) => {

  return (
   <>
    <Card  sx={{margin:"20px"}}>
      <CardHeader
        avatar={
          <Avatar sx={{ bgcolor: "red" }} aria-label="recipe" src={image_location}/>
        }
         title={msg.whoSendMsg.username} 
        subheader={msg.timestamp && ReturnDate(msg.timestamp)}
      />

      <CardMedia
        component="img"
        height="5%"
        width="5%"
        crossOrigin="anonymous"
        image={msg.uploadedImage.imageLocaton} 
       alt="alternative of image =>it's an image " 
      />
      <CardContent>
        <Typography variant="body2" sx={{fontSize:"20px"}} color="text.secondary">
        {msg.uploadedImage.imageCaption ? msg.uploadedImage.imageCaption : `khaled is say what=> no image caption`}
        </Typography>
      </CardContent>
    </Card>
    </>
  )
}

export default ChatShowUploadedImageComponent



 